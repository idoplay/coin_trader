...
class SpiderEngine(Abstract):
    ...
    def run_worker(self, data, run_function):
        requests = makeRequests(run_function, data, self.print_result, self.handle_exception)
    ...
    def http_post(self, url, params):
        ...
        httpRequest = SafeSession()
        httpRequest.headers.update({'User-Agent': 'Mozilla/5.0 (X11; Linux i686; U;) Gecko/20070322 Kazehakase/0.4.5'})
        r = httpRequest.post(url, data=json.dumps(params))
        r.encoding = 'utf-8'
        return r.text
